...
#------------------------------------------------------------------------------
run_pipe "${GSTLAUNCH} ${DEBUG} filesrc location=$fileName ! typefind !  qtdemux name=demux demux.audio_00 ! queue max-size-buffers=8000 max-size-time=0 max-size-bytes=0 ! typefind ! ${audio_plugin} ${audio_plugin_args} ! ${audio_sink} demux.video_00 ! typefind ! ${video_plugin} ${video_plugin_args} ! ${video_sink} ${rotation_opts}"
exit $?
