...
#DEBUG="--gst-debug-no-color --gst-debug=TIViddec2:5 --gst-debug=TIAuddec:5"
#DEBUG="--gst-debug-no-color --gst-debug=queue:5 --gst-debug=TIViddec2:5"
#DEBUG="--gst-debug-no-color --gst-debug=GST_PLUGIN_LOADING:5"
#DEBUG=-v


gst-launch ${DEBUG} filesrc location=$1 ! typefind ! avidemux name=demux \
 demux.audio_00 ! queue max-size-buffers=1200 max-size-time=0 max-size-bytes=0 ! typefind ! TIAuddec1 ! audioconvert ! 'audio/x-raw-int, width=16, depth=16' !  alsasink  \
 demux.video_00 ! queue max-size-buffers=60 ! typefind ! TIViddec2 !  TIDmaiVideoSink displayDevice=/dev/video1 displayStd=v4l2 videoStd=VGA videoOutput=LCD rotation=90
