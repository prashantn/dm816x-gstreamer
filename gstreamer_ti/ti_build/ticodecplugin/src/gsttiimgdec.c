...
static GstFlowReturn gst_tiimgdec_chain(GstPad * pad, GstBuffer * buf)
{
     ...
     */
    if (gst_tithread_check_status(imgdec, TIThread_CODEC_ABORTED,
            checkResult)) {
...
static gboolean gst_tiimgdec_init_image(GstTIImgdec *imgdec)
{
    ...
    gst_tithread_set_status(imgdec, TIThread_CODEC_CREATED);
...
static gboolean gst_tiimgdec_exit_image(GstTIImgdec *imgdec)
{
    ...
    if (gst_tithread_check_status(
            imgdec, TIThread_CODEC_CREATED, checkResult)) {
        GST_LOG("shutting down decode thread\n");

        Rendezvous_force(imgdec->waitOnDecodeThread);
...
static gboolean gst_tiimgdec_codec_stop (GstTIImgdec  *imgdec)
{
    ...
    if (imgdec->hOutBufTab) {
        ...
        gst_tidmaibuftab_unref(imgdec->hOutBufTab);
        imgdec->hOutBufTab = NULL;
    }
...
static gboolean gst_tiimgdec_codec_start (GstTIImgdec  *imgdec)
{
    ...
    imgdec->hOutBufTab = gst_tidmaibuftab_new(imgdec->numOutputBufs,
        Idec_getOutBufSize(imgdec->hIe), BufferGfx_getBufferAttrs(&gfxAttrs));
...
static void* gst_tiimgdec_decode_thread(void *arg)
{
    ...
    while (TRUE) {
        ...
        if (!(hDstBuf = gst_tidmaibuftab_get_buf(imgdec->hOutBufTab))) {
         ...
         * gst_buffer_unref().
         */
        outBuf = gst_tidmaibuffertransport_new(hDstBuf, imgdec->hOutBufTab);
...
thread_failure:
    ...
    gst_tithread_set_status(imgdec, TIThread_CODEC_ABORTED);
...
thread_exit:
    ...
    bufIdx = BufTab_getNumBufs(GST_TIDMAIBUFTAB_BUFTAB(imgdec->hOutBufTab));

    while (bufIdx-- > 0) {
        Buffer_Handle hBuf = BufTab_getBuf(
            GST_TIDMAIBUFTAB_BUFTAB(imgdec->hOutBufTab), bufIdx);
