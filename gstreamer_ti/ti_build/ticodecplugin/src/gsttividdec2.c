...
static void gst_tividdec2_init(GstTIViddec2 *viddec2, GstTIViddec2Class *gclass)
{
    ...
    gst_pad_set_getcaps_function(
        viddec2->sinkpad, GST_DEBUG_FUNCPTR(gst_pad_proxy_getcaps));
    ...
    gst_pad_set_getcaps_function(
        viddec2->srcpad, GST_DEBUG_FUNCPTR(gst_pad_proxy_getcaps));
...
static gboolean gst_tividdec2_init_video(GstTIViddec2 *viddec2)
{
    ...
    viddec2->circBuf = gst_ticircbuffer_new(Vdec2_getInBufSize(viddec2->hVd));
...
thread_failure:
     ...
     */
    if (encDataWindow) {
        gst_ticircbuffer_data_consumed(viddec2->circBuf, encDataWindow, 0, 0);
    }
