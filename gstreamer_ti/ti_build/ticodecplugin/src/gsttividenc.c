...
/* Constants */
#define gst_tividenc_CODEC_FREE 0x2
                ...
                const GValue *value, GParamSpec *pspec)
{
    ...
    switch (prop_id) {
        ...
        case PROP_BYTE_STREAM:
            videnc->byteStream = g_value_get_boolean(value);
            GST_LOG("setting \"byteStream\" to \"%s\"\n",
                videnc->byteStream ? "TRUE" : "FALSE");
            break;
                    ...
                    GstTIVidenc *videnc, Buffer_Handle hBuf)
{
    ...
    /* Create H264 cap */
    if (h264Codec && (!strcmp(h264Codec->CE_CodecName, videnc->codecName))) {
        caps =
        ...
        if (!videnc->byteStream) {
            gst_caps_set_simple(caps, "codec_data", GST_TYPE_BUFFER, 
                videnc->codec_data, (char*)NULL);
        }
    ...
    /* Create MPEG4 cap */
    if (mpeg4Codec && (!strcmp(mpeg4Codec->CE_CodecName, videnc->codecName))) {
        gint mpegversion = 4;
...
static gboolean gst_tividenc_exit_video(GstTIVidenc *videnc)
{
    ...
    if (videnc->codec_data) {
        GST_LOG("freeing codec_data buffer\n");
        gst_buffer_unref(videnc->codec_data);
        videnc->codec_data = NULL;
    }
...
static gboolean gst_tividenc_codec_stop (GstTIVidenc *videnc)
{
    ...
    if (videnc->hOutBufTab) {
        ...
        gst_tidmaibuftab_unref(videnc->hOutBufTab);
        videnc->hOutBufTab = NULL;
    }
...
static gboolean gst_tividenc_codec_start (GstTIVidenc *videnc)
{
    ...
    gfxAttrs.bAttrs.useMask = gst_tividenc_CODEC_FREE;

    if (videnc->numOutputBufs == 0) {
        videnc->numOutputBufs = 3;
    }

    videnc->hOutBufTab = gst_tidmaibuftab_new(videnc->numOutputBufs,
        Venc_getOutBufSize(videnc->hVe), BufferGfx_getBufferAttrs(&gfxAttrs));
...
/******************************************************************************
 * gst_tividenc_populate_codec_header
 *  This function populates codec_data field for H.264.
 *****************************************************************************/
void gst_tividenc_populate_codec_header (GstTIVidenc *videnc, 
    Buffer_Handle hDstBuf)
{
    if (gst_is_h264_encoder(videnc->codecName)) {
        if (!videnc->byteStream) {
    ...
    GstBuffer *outBuf)
{
    ...
    if (gst_is_h264_encoder(videnc->codecName)) {
        ...
        if ((!videnc->byteStream) && (videnc->codec_data)) {
...
static void* gst_tividenc_encode_thread(void *arg)
{
    ...
    while (TRUE) {
        ...
        if (!(hDstBuf = gst_tidmaibuftab_get_buf(videnc->hOutBufTab))) {
        ...
        gst_tividenc_populate_codec_header(videnc, hDstBuf);
        ...
        Buffer_freeUseMask(hDstBuf, gst_tividenc_CODEC_FREE);
...
thread_exit: 
    ...
    bufIdx = BufTab_getNumBufs(GST_TIDMAIBUFTAB_BUFTAB(videnc->hOutBufTab));

    while (bufIdx-- > 0) {
        Buffer_Handle hBuf = BufTab_getBuf(
            GST_TIDMAIBUFTAB_BUFTAB(videnc->hOutBufTab), bufIdx);
        Buffer_freeUseMask(hBuf, gst_tividenc_CODEC_FREE);
    }
