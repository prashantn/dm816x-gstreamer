...
static gboolean gst_tiimgenc1_exit_image(GstTIImgenc1 *imgenc1)
{
    ...
    if (gst_tithread_check_status(
         ...
         */
        if (Fifo_put(imgenc1->hInFifo,&gst_ti_flush_fifo) < 0) {
    ...
    /* NOTE: Shutting encode decode thread frees the circular buffer being used
    ...
    if (imgenc1->waitOnBufTab) {
        Rendezvous_delete(imgenc1->waitOnBufTab);
        imgenc1->waitOnBufTab = NULL;
    }
...
static gboolean gst_tiimgenc1_codec_stop (GstTIImgenc1  *imgenc1)
{
    ...
    if (imgenc1->hOutBufTab) {
        Int numBufs = BufTab_getNumBufs(imgenc1->hOutBufTab);

        GST_LOG("Re-claiming %d output buffers\n", numBufs);

        for (; numBufs > 0; numBufs--) {
            Buffer_Handle hBuf = BufTab_getFreeBuf(imgenc1->hOutBufTab);

            if (hBuf == NULL) {
                GST_LOG("Waiting on output buffer to be released\n");
                Rendezvous_meet(imgenc1->waitOnBufTab);
                hBuf = BufTab_getFreeBuf(imgenc1->hOutBufTab);
            ...
            Rendezvous_reset(imgenc1->waitOnBufTab);
        }
...
static void* gst_tiimgenc1_encode_thread(void *arg)
{
    ...
    while (TRUE) {
         ...
         * gst_buffer_unref().
         */
        outBuf = gst_tidmaibuffertransport_new(hDstBuf, imgenc1->waitOnBufTab);
